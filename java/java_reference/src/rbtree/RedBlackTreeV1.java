...
/**
 * this is the original version red black tree implementation.
 * the insert part ref Goodrich 6th 11.6.1
 * the delete part ref L17-Red-Black-Trees-clean.pptx from IIT (<a href="https://www.youtube.com/watch?v=JRsN4Oz36QU&list=PLBF3763AF2E1C572F&index=15">Lecture - 14 Red Black Trees</a>)
 * the delete case 2.2.2 eludes me when it comes to how to recursively proceed up the tree to re-balance, to resolve this issue
 * I referred to (<a href="https://www.happycoders.eu/algorithms/red-black-tree-java/">RED-BLACK TREE
 * (FULLY EXPLAINED, WITH JAVA CODE)</a>) deletion case 4. I realized that, in step 1 delete the node(maybe done by swap); in
 * step 2 fix the tree, there are six cases to cope with, which involves only rotation and recoloring, no more deleting nodes.
 * so case 2.2.2 can be proceeded up by recursively matching the parent to the six cases until root, just assume the whole subtree
 * as a whole that needs to be re-balanced
 *
 * so there are many aspects need refined:
 * 1. first do the remove
 * 2. fix the remove (re-balance & recolor)
 * 3. abstract the tedious deletion matching with rotate-left/right
 * 4. add parent pointer for node?
 * 5. simplify, remove generics?
 */
public class RedBlackTreeV1<T extends Comparable<T>> {
    ...
    public RedBlackTreeV1(Node.ColorTreeNode<T> node) {
        this.root =  new Node.ColorTreeNode<>(node.key, node.left, node.right, BLACK);
    }

    public RedBlackTreeV1(T v) {
    ...
    private static void insertExample() {
        // Figure 11.34
        RedBlackTreeV1<Integer> bst = new RedBlackTreeV1<>();
        bst.insert(4);
